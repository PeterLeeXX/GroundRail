...
**🔎 只读了局部代码就开始给方案。** GroundRail 要求先建立与风险匹配的最小充分 `Fact`，让重要结论能够追溯到源码、配置、运行结果或外部契约。

**🧪 猜测在阶段之间逐渐变成“事实”。** 调查中的观察、证据支持的结论和计划中的行动必须分开；未知保留为证据缺口，不能从 Fact 进入 Plan、再进入 Execute 时被悄悄洗成确定答案。

**🧭 Agent 替用户完成产品或风险取舍。** 用户保留需求、产品行为、范围、成本、安全和不可逆选择；模型依据事实和仓库惯例自主决定普通实现细节。

**🔍 审查漏掉关键问题，或反复纠缠低价值建议。** Review 只针对稳定工件，在干净上下文中沿四个审查轴检查，并用 P0/P1/P2 控制优先级和噪声。

**🧳 长任务让主对话被旧实现细节占满。** 稳定工件、结果式委派和安全 Handoff 负责外置状态，让主 Agent 把上下文留给连续判断、冲突消解和最终综合。
